...
export async function POST(request: NextRequest) {
  ...
  // 2. 요청자가 admin인지 확인
  const callerSnap = await adminDb.collection('profiles').doc(callerUid).get()
  ...
  // 4. 대상 유저가 존재하는지 확인
  const targetSnap = await adminDb.collection('profiles').doc(targetUserId).get()
  if (!targetSnap.exists) {
    return NextResponse.json({ error: '대상 유저를 찾을 수 없습니다.' }, { status: 404 })
  }

  // 5. 역할 업데이트
  await adminDb.collection('profiles').doc(targetUserId).update({ role })
